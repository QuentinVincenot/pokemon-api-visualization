...
function display_search_suggestions() {
    ...
    console.log(POKEMON_DATA.length);
    console.log(POKEMON_SORTED_NAMES.length);
    ...
    while(matching_names_number < 5) {
        if(POKEMON_SORTED_NAMES[current_search_id].startsWith(search_bar_content)) {
    ...
    /*while(matching_names_number < 5 && current_search_id < POKEMON_SORTED_NAMES.length) {
        console.log(search_bar_content, " * ", POKEMON_SORTED_NAMES[current_search_id]);
    ...
    //console.log('Found :', matching_names);
}
